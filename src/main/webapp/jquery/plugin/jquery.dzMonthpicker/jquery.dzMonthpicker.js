(function($) {
	...
	$.fn.dzMonthpicker = function (month) {
		
		//CalAddCss();
		initCal({id:$(month).attr("id"),type:"mon",min:2009, max:2019,icon:"n"});
	...
	initCal = function(obj){
		...
		var iconUrl    = obj.iconUrl == null ? "jquery/jquery-ui-1.11.0.custom/images/btn_cal_normal.gif" : obj.iconUrl ;
	...
	// 달력 폼 그리기 
	drawCalForm = function(id,tId,calType,min,max,todayYN,iconYN,imageUrl){ // tId = 입력창ID				
		
		if(iconYN == "y"){
			$(document.body).append("<div id='"+ id +"Icon' style='position:absolute'><img id='"+ id +"IconImg' src='"+ getContextPath()+'/'+imageUrl +"'><div>");
			//$(document.body).append("<div id='"+ id +"Icon' style='position:absolute'><img id='"+ id +"IconImg' class='calBtnIcon'><div>");
		}
		...
		// 상단 년,달 이동 
		var html  = "<div class='calTitle'><table width='100%'><tr style='height:25px;'><td id='"+ id +"before' width='10%'  style='font-size: 10px;padding-left:4px;padding-top:4px;'><img src='"+getContextPath()+"/jquery/plugin/jquery.dzMonthpicker/images/btn_cal_pre_normal.png' /></td><td width='80%' align='center'>";
		...
		html += "</td><td id='"+ id +"next' width='10%' style='font-size: 10px;padding-left:4px;padding-top:4px;'><img src='"+getContextPath()+"/jquery/plugin/jquery.dzMonthpicker/images/btn_cal_next_normal.png' /></td></tr></table></div>";
	...
	// 날짜변경(달변경) 달력내용 그리기
	makeCal = function(id,dates,splitKey){				
	...
	makeCalMon = function(id,dates,splitKey){
	...
	// 날짜 유효성 체크
	isValidCheck = function(strDate,splitKey,todayCheck){
		var date = strDate;
	...
	calDate = function(year,month,adjust,isValidDate){
	...
	// 달력위치변경		
	calPosition = function(target,id){
